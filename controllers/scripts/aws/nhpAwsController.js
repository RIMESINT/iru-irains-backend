    ...
    try {
        ...
        const result = await client.query(`
            WITH aws AS (
                SELECT *, ${AWS_DAY} AS aws_day
                FROM observations_aws_nhp
                WHERE dat BETWEEN ($1::date - INTERVAL '1 day') AND $1::date ${districtFilter}
            )
            SELECT
                aws_day AS dat,
...
//    POST /api/v1/nhp-aws/station-slots
//    Body: { date? }
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
exports.fetchStationSlots = async (req, res) => {
    try {
        ...
        const query = `
            ...
            slots AS (
                SELECT
                    ...
                    ist_time, rainfall, rainfall_daily, temp, feel_like, rh, winds, windd, slp,
            ...
            totals AS (
                SELECT
                    ...
                    NULL::numeric AS rainfall_daily, NULL::numeric AS temp, NULL::numeric AS feel_like,
                    NULL::numeric AS rh, NULL::numeric AS winds, NULL::numeric AS windd, NULL::numeric AS slp,
                    'total'::text AS row_type, 1 AS sort_order
                FROM aws
                GROUP BY id, station, district, state
            )
            SELECT id, station, district, state,
                   ist_time, rainfall, rainfall_daily, temp, feel_like, rh, winds, windd, slp,
                   row_type
            FROM slots
            UNION ALL
        ...
        res.status(200).json({
            success: true,
            message: "NHP AWS station slots fetched successfully",
            data: result.rows
        });

    } catch (error) {
        console.error("[NHP AWS] fetchStationSlots error:", error);
...
// NHP has no block column; normals are aggregated from blockâdistrict.
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
exports.fetchDepartureForAPIexport = async (req, res) => {
    try {
        ...
        res.status(200).json({
            success: true,
            message: "NHP AWS departure data fetched",
            fromDate,
            toDate,
            data
        });
