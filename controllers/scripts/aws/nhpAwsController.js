...
exports.fetchDailyData = async (req, res) => {
    try {
        ...
        const result = await client.query(`
            ...
            GROUP BY dat, district, state, id, station
...
exports.fetchHourlyData = async (req, res) => {
    try {
        ...
        const result = await client.query(`
            ...
            GROUP BY dat, hour, district, state, id, station
...
exports.fetchDistrictSummary = async (req, res) => {
    try {
        ...
        const result = await client.query(`
            ...
            FROM (
                SELECT dat, district, state, id,
                    SUM(rainfall) AS daily_rain,
                    AVG(temp)     AS avg_temp
                FROM observations_aws_nhp
                WHERE dat = $1
                GROUP BY dat, district, state, id
            ) AS station_daily
            GROUP BY dat, district, state
