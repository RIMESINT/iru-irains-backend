    ...
    try {
        ...
        const departureQuery = `
            ...
            WHERE bd.from_date BETWEEN $1 AND $2
                AND bd.from_date = bd.to_date
                AND bd.block_id = $3
            ORDER BY bd.departure DESC
            LIMIT $4;
        `;

        const [actualResult, departureResult] = await Promise.all([
            client.query(actualQuery, [startDate, endDate, block_code, topN]),
            client.query(departureQuery, [startDate, endDate, block_code, topN]),
        ]);
    ...
    try {
        ...
        const departureQuery = `
            ...
            WHERE dd.from_date BETWEEN $1 AND $2
                AND dd.from_date = dd.to_date
                AND dd.district_id = $3
            ORDER BY dd.departure DESC
            LIMIT $4;
        `;

        const [actualResult, departureResult] = await Promise.all([
            client.query(actualQuery, [startDate, endDate, district_code, topN]),
            client.query(departureQuery, [startDate, endDate, district_code, topN]),
        ]);
    ...
    try {
        ...
        const departureQuery = `
            ...
            WHERE sd.from_date BETWEEN $1 AND $2
                AND sd.from_date = sd.to_date
                AND sd.subdivision_id = $3
            ORDER BY sd.departure DESC
            LIMIT $4;
        `;

        const [actualResult, departureResult] = await Promise.all([
            client.query(actualQuery, [startDate, endDate, subdivision_code, topN]),
            client.query(departureQuery, [startDate, endDate, subdivision_code, topN]),
        ]);
...
exports.fetchTopNStates = async (req, res) => {
    try {
        ...
        const [actualResult, departureResult] = await Promise.all([
            client.query(actualQuery, [startDate, endDate, state_code, topN]),
            client.query(departureQuery, [startDate, endDate, state_code, topN]),
        ]);
    ...
    try {
        ...
        const departureQuery = `
            ...
            WHERE rd.from_date BETWEEN $1 AND $2
                AND rd.from_date = rd.to_date
                AND rd.region_id = $3
            ORDER BY rd.departure DESC
            LIMIT $4;
        `;

        const [actualResult, departureResult] = await Promise.all([
            client.query(actualQuery, [startDate, endDate, region_code, topN]),
            client.query(departureQuery, [startDate, endDate, region_code, topN]),
        ]);
    ...
    try {
        ...
        const departureQuery = `
            ...
            WHERE cd.from_date BETWEEN $1 AND $2
                AND cd.from_date = cd.to_date
                AND cd.country_id = $3
            ORDER BY cd.departure DESC
            LIMIT $4;
        `;

        const [actualResult, departureResult] = await Promise.all([
            client.query(actualQuery, [startDate, endDate, country_code, topN]),
            client.query(departureQuery, [startDate, endDate, country_code, topN]),
        ]);

        res.status(200).json({
            success: true,
            message: `Top ${topN} records for country_code ${country_code} by actual rainfall (filtered for actual < 999) and departure`,
            data: {
                topByActual: actualResult.rows,
                topByDeparture: departureResult.rows,
            },
