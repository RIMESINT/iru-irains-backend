...
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// 7. STATION SLOTS â all 15-min slots + total row per station for one AWS day
...
// POST /api/v1/tamilnadu-aws/actual-departure
exports.fetchActualDeparture = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: "TamilNadu AWS actual departure fetched", data });
    } catch (error) {
        console.error("[TN AWS] fetchActualDeparture:", error);
        res.status(500).json({ success: false, message: error.message });
    }
};

// POST /api/v1/tamilnadu-aws/departure-analysis
exports.fetchDepartureAnalysis = async (req, res) => {
    try {
        ...
        res.status(200).json({
            success: true, message: "TamilNadu AWS departure analysis fetched",
            data: {
                categoryCounts,
        ...
        console.error("[TN AWS] fetchDepartureAnalysis:", error);
        res.status(500).json({ success: false, message: error.message });
    }
};

// POST /api/v1/tamilnadu-aws/departure-export
exports.fetchDepartureForAPIexport = async (req, res) => {
    try {
        ...
        res.status(200).json({
            success: true,
            message: "TamilNadu AWS departure data fetched",
            fromDate,
            toDate,
            data
        });
    } catch (error) {
        console.error("[TN AWS] fetchDepartureForAPIexport:", error);
        res.status(500).json({ success: false, message: error.message });
    }
};
