...
// ─────────────────────────────────────────────────────────────────────────────
// 1. DAILY
//    POST /api/v1/telangana-aws/daily
...
exports.fetchDailyData = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: "Telangana Daily data fetched", data: result.rows });

    } catch (error) {
        console.error("[TG AWS] fetchDailyData:", error);
...
// ─────────────────────────────────────────────────────────────────────────────
// 2. HOURLY
//    POST /api/v1/telangana-aws/hourly
...
exports.fetchHourlyData = async (req, res) => {
    try {
        ...
        const result = await client.query(query, params);
        res.status(200).json({ success: true, message: "Telangana Hourly data fetched", data: result.rows });

    } catch (error) {
        console.error("[TG AWS] fetchHourlyData:", error);
        res.status(500).json({ success: false, message: error.message });
    }
};
...
// ─────────────────────────────────────────────────────────────────────────────
// 3. SLOT
//    POST /api/v1/telangana-aws/slot
...
exports.fetchSlotData = async (req, res) => {
    try {
        ...
        const result = await client.query(query, params);
        res.status(200).json({ success: true, message: "Telangana Slot data fetched", data: result.rows });

    } catch (error) {
        console.error("[TG AWS] fetchSlotData:", error);
        res.status(500).json({ success: false, message: error.message });
    }
};
...
// ─────────────────────────────────────────────────────────────────────────────
// 4. CUMULATIVE
//    POST /api/v1/telangana-aws/cumulative
//    Body: { startDate?, endDate?, district? }
// ─────────────────────────────────────────────────────────────────────────────
exports.fetchCumulativeData = async (req, res) => {
    try {
        ...
        const result = await client.query(query, params);
        res.status(200).json({ success: true, message: "Telangana Cumulative data fetched", data: result.rows });

    } catch (error) {
        console.error("[TG AWS] fetchCumulativeData:", error);
        res.status(500).json({ success: false, message: error.message });
    }
};
...
// ─────────────────────────────────────────────────────────────────────────────
// 5. DISTRICT SUMMARY
//    POST /api/v1/telangana-aws/district-summary
//    Body: { date? }
// ─────────────────────────────────────────────────────────────────────────────
exports.fetchDistrictSummary = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: "Telangana District summary fetched", data: result.rows });

    } catch (error) {
        console.error("[TG AWS] fetchDistrictSummary:", error);
        res.status(500).json({ success: false, message: error.message });
    }
};
...
//    POST /api/v1/telangana-aws/station-slots
//    Body: { date? }
// ─────────────────────────────────────────────────────────────────────────────
exports.fetchStationSlots = async (req, res) => {
    try {
        ...
        res.status(200).json({
            success: true,
            message: "Telangana AWS station slots fetched successfully",
            data: result.rows
        });

    } catch (error) {
        console.error("[TG AWS] fetchStationSlots error:", error);
...
const fetchBetweenDates = async (startDate, endDate) => {
    const query = `
        ...
        FROM (
            ...
            FROM (
                SELECT block, district, state, id, ${AWS_DAY} AS dat, MAX(rainfall) AS station_rf
                FROM observations_aws_telangana
                WHERE dat BETWEEN ($1::date - INTERVAL '1 day') AND $2::date
                  AND ${AWS_DAY} BETWEEN $1::date AND $2::date
...
// POST /api/v1/telangana-aws/actual-departure
exports.fetchActualDeparture = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: "Telangana AWS actual departure fetched", data });
    } catch (error) {
        console.error("[TG AWS] fetchActualDeparture:", error);
        res.status(500).json({ success: false, message: error.message });
    }
};

// POST /api/v1/telangana-aws/departure-analysis
exports.fetchDepartureAnalysis = async (req, res) => {
    try {
        ...
        res.status(200).json({
            success: true, message: "Telangana AWS departure analysis fetched",
            data: {
                categoryCounts,
        ...
        console.error("[TG AWS] fetchDepartureAnalysis:", error);
        res.status(500).json({ success: false, message: error.message });
    }
};

// POST /api/v1/telangana-aws/departure-export
exports.fetchDepartureForAPIexport = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: "Telangana AWS departure data fetched", data });
    } catch (error) {
        console.error("[TG AWS] fetchDepartureForAPIexport:", error);
        res.status(500).json({ success: false, message: error.message });
    }
};
