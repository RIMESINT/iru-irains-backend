...
exports.fetchDailyData = async (req, res) => {
    try {
        ...
        const query = `
            ...
            SELECT
                aws_day AS dat,
                district, id, station, station_type,
            ...
            GROUP BY aws_day, district, id, station, station_type
            ORDER BY aws_day, district, total_rainfall DESC
        `;

        const result = await client.query(query, params);
        res.status(200).json({ success: true, message: "Mizoram Daily data fetched", data: result.rows });

    } catch (error) {
        console.error("[MIZ AWS] fetchDailyData:", error);
...
// ─────────────────────────────────────────────────────────────────────────────
// 2. HOURLY
//    POST /api/v1/mizoram-aws/hourly
    ...
    try {
        ...
        const query = `
            ...
            SELECT
                ...
                COUNT(*)                            AS readings_count
            FROM aws
            WHERE aws_day = $1::date
              ${hourFilter}
            GROUP BY aws_day, EXTRACT(HOUR FROM time)::INT, district, id, station, station_type
            ORDER BY hour, district, total_rainfall DESC
        `;

        const result = await client.query(query, params);
        res.status(200).json({ success: true, message: "Mizoram Hourly data fetched", data: result.rows });

    } catch (error) {
        console.error("[MIZ AWS] fetchHourlyData:", error);
        res.status(500).json({ success: false, message: error.message });
    }
};
...
exports.fetchSlotData = async (req, res) => {
    try {
        ...
        const query = `
            ...
            FROM observations_aws_mizoram
            WHERE dat = $1 AND time = $2
              ${districtFilter}
            ORDER BY district, rainfall DESC
        `;

        const result = await client.query(query, params);
        res.status(200).json({ success: true, message: "Mizoram Slot data fetched", data: result.rows });

    } catch (error) {
        console.error("[MIZ AWS] fetchSlotData:", error);
        res.status(500).json({ success: false, message: error.message });
    }
};
...
// ─────────────────────────────────────────────────────────────────────────────
// 4. CUMULATIVE
//    POST /api/v1/mizoram-aws/cumulative
//    Body: { startDate?, endDate?, district? }
// ─────────────────────────────────────────────────────────────────────────────
exports.fetchCumulativeData = async (req, res) => {
    try {
        ...
        const result = await client.query(query, params);
        res.status(200).json({ success: true, message: "Mizoram Cumulative data fetched", data: result.rows });

    } catch (error) {
        console.error("[MIZ AWS] fetchCumulativeData:", error);
        res.status(500).json({ success: false, message: error.message });
    }
};
...
// ─────────────────────────────────────────────────────────────────────────────
// 5. DISTRICT SUMMARY
//    POST /api/v1/mizoram-aws/district-summary
//    Body: { date? }
// ─────────────────────────────────────────────────────────────────────────────
exports.fetchDistrictSummary = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: "Mizoram District summary fetched", data: result.rows });

    } catch (error) {
        console.error("[MIZ AWS] fetchDistrictSummary:", error);
        res.status(500).json({ success: false, message: error.message });
    }
};
...
//    POST /api/v1/mizoram-aws/station-slots
//    Body: { date? }
// ─────────────────────────────────────────────────────────────────────────────
exports.fetchStationSlots = async (req, res) => {
    try {
        ...
        const query = `
            ...
            slots AS (
                SELECT
                    ...
                    ist_time, rainfall, rainfall_hourly, temp, rh, slp, winds, windd,
            ...
            totals AS (
                SELECT
                    ...
                    NULL::numeric AS rainfall_hourly, NULL::numeric AS temp, NULL::numeric AS rh,
                    NULL::numeric AS slp, NULL::numeric AS winds, NULL::numeric AS windd,
                    NULL::numeric AS soil_moisture, NULL::numeric AS soil_temp, NULL::numeric AS solar_radiation,
                    'total'::text AS row_type, 1 AS sort_order
                FROM aws
                GROUP BY id, station, station_type, state, district
            )
            SELECT id, station, station_type, state, district,
                   ist_time, rainfall, rainfall_hourly, temp, rh, slp, winds, windd,
                   soil_moisture, soil_temp, solar_radiation,
                   row_type
            FROM slots
        ...
        res.status(200).json({
            success: true,
            message: "Mizoram AWS station slots fetched successfully",
            data: result.rows
        });

    } catch (error) {
        console.error("[MIZ AWS] fetchStationSlots error:", error);
...
// Mizoram has no block column; normals are aggregated from block→district.
// ─────────────────────────────────────────────────────────────────────────────
...
// POST /api/v1/mizoram-aws/actual-departure
exports.fetchActualDeparture = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: "Mizoram AWS actual departure fetched", data });
    } catch (error) {
        console.error("[MIZ AWS] fetchActualDeparture:", error);
        res.status(500).json({ success: false, message: error.message });
    }
};

// POST /api/v1/mizoram-aws/departure-analysis
exports.fetchDepartureAnalysis = async (req, res) => {
    try {
        ...
        res.status(200).json({
            success: true, message: "Mizoram AWS departure analysis fetched",
            data: {
                categoryCounts,
        ...
        console.error("[MIZ AWS] fetchDepartureAnalysis:", error);
        res.status(500).json({ success: false, message: error.message });
    }
};

// POST /api/v1/mizoram-aws/departure-export
exports.fetchDepartureForAPIexport = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: "Mizoram AWS departure data fetched", data });
    } catch (error) {
        console.error("[MIZ AWS] fetchDepartureForAPIexport:", error);
        res.status(500).json({ success: false, message: error.message });
    }
};
