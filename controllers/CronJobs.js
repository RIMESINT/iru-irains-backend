...
const { fetchAndStoreUP } = require("../controllers/scripts/aws/upAwsFetcher"); // ✅
...
// UP AWS — every 15 minutes all day
const upAwsJobs = ['0 * * * *', '15 * * * *', '30 * * * *', '45 * * * *'].map((pattern) =>
    schedule.scheduleJob(pattern, async () => {
        console.log(`[UP AWS CRON] Running at ${new Date().toLocaleString()}`);
        try {
            await fetchAndStoreUP();
        } catch (error) {
            console.error("[UP AWS CRON] Failed:", error.message);
        }
    })
);
...
(async () => {
    console.log("[UP AWS] Initial fetch on server start...");
    try {
        await fetchAndStoreUP();
    } catch (error) {
        console.error("[UP AWS] Initial fetch failed:", error.message);
    }
})();